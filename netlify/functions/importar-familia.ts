...
const handler: Handler = async (event: HandlerEvent) => {
  ...
  if (perfilActorError || sucursalActorError) {
    logServerError(event, {
      endpoint: ENDPOINT,
      operation: 'validate_actor_scope',
      statusCode: 502,
  ...
  if (errFamilia) {
    ...
    return json(502, { success: false, error: 'No se pudo validar la familia del reporte.' })
  }
  if (!familia) {
  ...
  for (const lote of enLotes(codArts)) {
    ...
    if (errCod || errEan) {
      logServerError(event, { endpoint: ENDPOINT, operation: 'load_catalog_candidates', statusCode: 502, error: errCod ?? errEan })
      return json(502, { success: false, error: 'No se pudo reconstruir la reconciliación.' })
    }
  ...
  for (const lote of enLotes(ids)) {
    const { data: estados, error: errEstados } = await supabase
      ...
      .in('producto_id', lote)
    if (errEstados) {
      logServerError(event, { endpoint: ENDPOINT, operation: 'load_local_product_state', statusCode: 502, error: errEstados })
      return json(502, { success: false, error: 'No se pudo cargar el estado local.' })
    }
  ...
  if (errAplicado) {
    ...
    logServerError(event, { endpoint: ENDPOINT, operation: 'aplicar_importacion_glaciar_familia_v1', statusCode: status, error: errAplicado })
    return json(status, {
      success: false,
      error: status === 403
        ? 'No tenés permiso para aplicar esta importación.'
