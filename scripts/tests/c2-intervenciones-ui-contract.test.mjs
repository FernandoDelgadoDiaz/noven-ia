...
assert.match(
  card,
  /const tieneIntervencionComercial = tieneRagActivo \|\| tieneOfertaCentralizada[\s\S]{0,240}?const accionesSinRag = tieneIntervencionComercial/,
  'una transferencia informada no puede ocultar una sugerencia RAG que todavía corresponda',
)
...
assert.match(
  modal,
  /setSalidaPendiente\(\{ observacionId, bajada: decision\.bajada \}\)\s+return/,
  'el modal debe detener el cierre hasta que el operador clasifique la caída',
)
assert.match(
  hook,
  /estado\?\.oferta_central_explicita\s*\n\s*\? estado\.oferta_centralizada/,
  'una oferta explícita finalizada debe prevalecer sobre el fallback histórico',
)
assert.match(
  hook,
  /!estado\.ultima_intervencion_evaluada/,
  'la compatibilidad histórica sólo puede considerar la intervención más reciente',
)
