...
// La carga masiva enruta por fuente, pero ambos fallos técnicos deben conservar
// el nombre exacto de la operación que falló en el logger estructurado.
...
for (const operation of [
  ...
  'load_expiries',
  'load_products',
  'load_sector_policies',
  'load_actions',
  'load_rag_tracking',
  'load_rag_interventions',
...
assert.doesNotMatch(analisis, /logServerError\([^)]*prompt/s)
assert.doesNotMatch(analisis, /logServerError\([^)]*bodyProveedor/s)

console.log('✓ Functions críticas usan observabilidad estructurada sin filtrar secretos')
