...
- **Las propiedades que NO estan en el texto de la migracion solo se verifican contra el catalogo real.** Ningun contrato de texto puede verlas: el SQL se ve bien, los grants estan, el fingerprint no marca nada raro. Paso dos veces en dos dias y las dos las cazo el verificador de exposicion contra la base efimera, no la lectura del diff:
  ...
  - **Un cuerpo plpgsql que compila no esta verificado; solo se verifica ejecutandolo.** PostgreSQL no analiza el cuerpo de una funcion plpgsql al crearla: guarda el texto. Un error de tipos, una funcion inexistente o una sobrecarga que no resuelve no impiden que la migracion aplique — quedan latentes hasta que alguien abre la pantalla que llama a esa funcion, con el esquema ya cambiado. En el bloque 3B fue `to_char` sobre un `time`, que no tiene sobrecarga: la migracion habria aplicado limpia y la bandeja zonal habria fallado en el primer uso. Se saco del camino con un cast explicito a `interval`, pero el punto no es ese cast: es que **aplicar sin error no es evidencia de que el cuerpo funcione**. Lo unico que lo prueba es ejecutar la funcion contra una base real —el replay descartable o el gate vivo—, no leerla ni aplicarla.

  Son tres propiedades de la misma familia: el permiso que nadie escribio, la reloption que el reemplazo borro, y el cuerpo que nadie ejecuto. Las tres se ven bien en el archivo. En los dos primeros el defecto era invisible en el codigo y grave en la base — el segundo habria dejado a `v_seguimiento_rag_actual`, que leen cinco lugares, sin filtrar por organizacion. Que el aislamiento multitenant sea hipotetico con un solo cliente no lo hace menos grave: con un solo tenant un fallo de aislamiento NO DA SINTOMA, y nadie se entera hasta que hay un segundo.

  Por eso el contrato de texto llega antes y el verificador contra el catalogo decide. Cuando una invariante dependa de un hecho del catalogo —permisos, reloptions, defaults del schema, lo que herede un objeto de otra migracion, o que un cuerpo plpgsql resuelva sus tipos— la verificacion tiene que correr contra una base real, no sobre el archivo.

- **Constatar un hecho y autorizar un cambio son permisos distintos.** Constatar lo puede hacer quien esta frente al producto; autorizar, quien tiene la jerarquia. En el codigo son dos guards y no se intercambian: una RPC de constatacion autoriza por ALCANCE (`noven_private.puede_ver_producto_sucursal`) y no lista roles; una de autorizacion lista roles explicitos (`ua.rol IN (...)`) y no se conforma con el alcance.

  El costo de confundirlos ya esta medido. Al cerrar la puerta lateral por la que el browser abria un RAG se reconstruyo solamente el camino de AUTORIZAR un cambio (`solicitar_cambio_rag`, que exige `rag_porcentaje IS NOT NULL`), y el primer RAG —que es constatar un descuento que la cadena ya puso en gondola— quedo pidiendo jerarquia. Nueve vencimientos se quedaron fuera del motor: no por falta de permiso, sino porque no habia camino. Y no da sintoma de error: la pantalla decia "Todavia no hay un RAG registrado", que es verdad, junto a ningun boton.

  Dos consecuencias practicas. La escala gobierna lo que se SUGIERE, nunca lo que se puede DECLARAR: si la cadena puso un porcentaje fuera de escala, eso ocurrio, y negarse a registrarlo no lo deshace —solo deja al sistema ciego sobre un producto intervenido. Y la frontera tiene contrato en las dos direcciones, con mutantes en las dos: `scripts/tests/frontera-constatar-autorizar-contract.test.mjs` falla si un constatador pide un rol, si un autorizador se conforma con el alcance, y si aparece una RPC nueva de las familias informar / finalizar / solicitar / ejecutar / verificar sin declarar de que lado esta.
